...
var attrOIDMap = map[radius.Type]radius.NameType{
	1:  {"3GPP-IMSI", 1, nil},
	2:  {"3GPP-Charging-ID", 5, nil},
	3:  {"3GPP-PDP-Type", 5, nil},
	4:  {"3GPP-Charging-Gateway-Address", 3, nil},
	5:  {"3GPP-GPRS-Negotiated-QoS-profile", 1, nil},
	6:  {"3GPP-SGSN-Address", 3, nil},
	7:  {"3GPP-GGSN-Address", 3, nil},
	8:  {"3GPP-IMSI-MCC-MNC", 1, nil},
	9:  {"3GPP-GGSN-MCC-MNC", 1, nil},
	10: {"3GPP-NSAPI", 1, nil},
	11: {"3GPP-Session-Stop-Indicator", 13, nil},
	12: {"3GPP-Selection-Mode", 1, nil},
	13: {"3GPP-Charging-Characteristics", 1, nil},
	14: {"3GPP-Charging-Gateway-IPv6-Address", 6, nil},
	15: {"3GPP-SGSN-IPv6-Address", 6, nil},
	16: {"3GPP-GGSN-IPv6-Address", 6, nil},
	17: {"3GPP-IPv6-DNS-Servers", 2, nil},
	18: {"3GPP-SGSN-MCC-MNC", 1, nil},
	19: {"3GPP-Teardown-Indicator", 13, nil},
	20: {"3GPP-IMEISV", 1, nil},
	21: {"3GPP-RAT-Type", 13, ThreeGPPRATType_GetValueString},
	22: {"3GPP-Location-Info", 2, nil},
	23: {"3GPP-MS-Time-Zone", 5, nil},
	24: {"3GPP-Camel-Charging-Info", 2, nil},
	25: {"3GPP-Packet-Filter", 2, nil},
	26: {"3GPP-Negotiated-DSCP", 13, nil},
	27: {"3GPP-Allocate-IP-Type", 13, ThreeGPPAllocateIPType_GetValueString},
}

var attrNameMap = map[string]radius.OIDType{
	"3GPP-IMSI":                          {1, 1, nil},
	"3GPP-Charging-ID":                   {2, 5, nil},
	"3GPP-PDP-Type":                      {3, 5, nil},
	"3GPP-Charging-Gateway-Address":      {4, 3, nil},
	"3GPP-GPRS-Negotiated-QoS-profile":   {5, 1, nil},
	"3GPP-SGSN-Address":                  {6, 3, nil},
	"3GPP-GGSN-Address":                  {7, 3, nil},
	"3GPP-IMSI-MCC-MNC":                  {8, 1, nil},
	"3GPP-GGSN-MCC-MNC":                  {9, 1, nil},
	"3GPP-NSAPI":                         {10, 1, nil},
	"3GPP-Session-Stop-Indicator":        {11, 13, nil},
	"3GPP-Selection-Mode":                {12, 1, nil},
	"3GPP-Charging-Characteristics":      {13, 1, nil},
	"3GPP-Charging-Gateway-IPv6-Address": {14, 6, nil},
	"3GPP-SGSN-IPv6-Address":             {15, 6, nil},
	"3GPP-GGSN-IPv6-Address":             {16, 6, nil},
	"3GPP-IPv6-DNS-Servers":              {17, 2, nil},
	"3GPP-SGSN-MCC-MNC":                  {18, 1, nil},
	"3GPP-Teardown-Indicator":            {19, 13, nil},
	"3GPP-IMEISV":                        {20, 1, nil},
	"3GPP-RAT-Type":                      {21, 13, ThreeGPPRATType_GetValueNumber},
	"3GPP-Location-Info":                 {22, 2, nil},
	"3GPP-MS-Time-Zone":                  {23, 5, nil},
	"3GPP-Camel-Charging-Info":           {24, 2, nil},
	"3GPP-Packet-Filter":                 {25, 2, nil},
	"3GPP-Negotiated-DSCP":               {26, 13, nil},
	"3GPP-Allocate-IP-Type":              {27, 13, ThreeGPPAllocateIPType_GetValueNumber},
}
...
func init() {
	attributemap.RegisterVendor(_ThreeGPP_VendorID, GetAttrName, GetAttrOID)
}
...
var ThreeGPPRATType_Strings = map[uint32]string{
	101: "IEEE-802.16e",
	102: "3GPP2-eHRPD",
	103: "3GPP2-HRPD",
	104: "3GPP2-1xRTT",
...
func ThreeGPPRATType_GetValueString(value uint32) (str string, err error) {
	str, ok := ThreeGPPRATType_Strings[value]
	if !ok {
		err = fmt.Errorf("value: %d not found in ThreeGPPRATType mapping", value)
	}
	return
}

func ThreeGPPRATType_GetValueNumber(value string) (str uint32, err error) {
	for k, v := range ThreeGPPRATType_Strings {
		if v == value {
			return k, nil
		}
	}
	err = fmt.Errorf("value: %s not found in ThreeGPPRATType mapping", value)
	return
}

func ThreeGPPLocationInfo_Add(p *radius.Packet, value []byte) (err error) {
	var a radius.Attribute
	a, err = radius.NewBytes(value)
	if err != nil {
		return
...
func ThreeGPPLocationInfo_AddString(p *radius.Packet, value string) (err error) {
	var a radius.Attribute
	a, err = radius.NewString(value)
	if err != nil {
		return
...
func ThreeGPPLocationInfo_Get(p *radius.Packet) (value []byte) {
	value, _ = ThreeGPPLocationInfo_Lookup(p)
	return
}

func ThreeGPPLocationInfo_GetString(p *radius.Packet) (value string) {
	value, _ = ThreeGPPLocationInfo_LookupString(p)
	return
}

func ThreeGPPLocationInfo_Gets(p *radius.Packet) (values [][]byte, err error) {
	var i []byte
...
func ThreeGPPLocationInfo_GetStrings(p *radius.Packet) (values []string, err error) {
	var i string
	for _, attr := range _ThreeGPP_GetsVendor(p, 22) {
		i = radius.String(attr)
		if err != nil {
...
func ThreeGPPLocationInfo_Lookup(p *radius.Packet) (value []byte, err error) {
...
func ThreeGPPLocationInfo_LookupString(p *radius.Packet) (value string, err error) {
	a, ok := _ThreeGPP_LookupVendor(p, 22)
	if !ok {
		err = radius.ErrNoAttribute
		return
...
func ThreeGPPLocationInfo_Set(p *radius.Packet, value []byte) (err error) {
	var a radius.Attribute
	a, err = radius.NewBytes(value)
	if err != nil {
		return
...
func ThreeGPPLocationInfo_SetString(p *radius.Packet, value string) (err error) {
	var a radius.Attribute
	a, err = radius.NewString(value)
	if err != nil {
		return
...
func ThreeGPPLocationInfo_Del(p *radius.Packet) {
	_ThreeGPP_DelVendor(p, 22)
}

type ThreeGPPMSTimeZone uint32

var ThreeGPPMSTimeZone_Strings = map[ThreeGPPMSTimeZone]string{}

func (a ThreeGPPMSTimeZone) String() string {
	if str, ok := ThreeGPPMSTimeZone_Strings[a]; ok {
		return str
	}
	return "ThreeGPPMSTimeZone(" + strconv.FormatUint(uint64(a), 10) + ")"
}

func ThreeGPPMSTimeZone_Add(p *radius.Packet, value ThreeGPPMSTimeZone) (err error) {
	a := radius.NewInteger(uint32(value))
	return _ThreeGPP_AddVendor(p, 23, a)
}

func ThreeGPPMSTimeZone_Get(p *radius.Packet) (value ThreeGPPMSTimeZone) {
	value, _ = ThreeGPPMSTimeZone_Lookup(p)
	return
}

func ThreeGPPMSTimeZone_Gets(p *radius.Packet) (values []ThreeGPPMSTimeZone, err error) {
	var i uint32
	for _, attr := range _ThreeGPP_GetsVendor(p, 23) {
		...
		values = append(values, ThreeGPPMSTimeZone(i))
	}
	return
}

func ThreeGPPMSTimeZone_Lookup(p *radius.Packet) (value ThreeGPPMSTimeZone, err error) {
	...
	value = ThreeGPPMSTimeZone(i)
	return
}

func ThreeGPPMSTimeZone_Set(p *radius.Packet, value ThreeGPPMSTimeZone) (err error) {
	a := radius.NewInteger(uint32(value))
...
var ThreeGPPAllocateIPType_Strings = map[uint32]string{
	0: "Do-Not-Allocate",
	1: "Allocate-IPv4-Address",
	2: "Allocate-IPv6-Prefix",
	3: "Allocate-IPv4-and-IPv6",
}

func ThreeGPPAllocateIPType_GetValueString(value uint32) (str string, err error) {
	str, ok := ThreeGPPAllocateIPType_Strings[value]
	if !ok {
		err = fmt.Errorf("value: %d not found in ThreeGPPAllocateIPType mapping", value)
	}
	return
}

func ThreeGPPAllocateIPType_GetValueNumber(value string) (str uint32, err error) {
	for k, v := range ThreeGPPAllocateIPType_Strings {
		if v == value {
			return k, nil
		}
	}
	err = fmt.Errorf("value: %s not found in ThreeGPPAllocateIPType mapping", value)
	return
}
